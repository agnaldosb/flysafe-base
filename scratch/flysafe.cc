...
void FlySafeSimulation(uint32_t nNodes, string simDate, char runMode, int nMalicious) {
  ...
  //wifi.SetStandard (WIFI_STANDARD_80211n_2_4GHZ); // Update on Mar 31, 2026 by Vinicius Trindade
  ...
  */
  ...
  for (it = Nodes.Begin(); it != Nodes.End(); it++) {
    ...
    */
    ...
    SinkApp->Setup(SinkBroadAddress, 1, 9999.99); // Vinicius - All nodes are ordinary
    i++;
  }
  ...
  for (it = Nodes.Begin(); it != Nodes.End(); it++) {
    ...
    */
    ...
    OnOffApp->Setup(InetSocketAddress(Ipv4Address("255.255.255.255"), 9),
                    1, 9999.99); // Vinicius - All nodes are ordinary
    i++;
