...
TypeId FlySafePacketSink::GetTypeId(void) {
  static TypeId tid =
      TypeId("ns3::FlySafePacketSink")
          ...
          /** 
           * @author Vinicius - MiM
           * @note Malicious UAV Implementation - Traced Callback: received messages can be used in MiM
           * @date Jul 07, 2025  
          */
...
void FlySafePacketSink::PacketReceived(Ptr<Socket> socket) {
  ...
  while ((packet = socket->RecvFrom(neighAdd))) {
    ...
    if (InetSocketAddress::IsMatchingType(neighAdd)) {
      ...
      /** 
       * @author Vinicius - MiM
       * @note Malicious UAV Implementation
       * @date Jul 07, 2025  
      */
...
void FlySafePacketSink::UpdateMyNeighborList(std::vector<ns3::MyTag::NeighInfos> neighInfos){
  ...
  for(auto n :neighInfos){
    if(n.ip != m_nodeIP){ // Avoid register in NL the node itself
...
/** 
   * @author Vinicius - MiM
   * @note Malicious UAV Implementation - Notify neighbor nodes (one hop away and non malicious) about a malicious node can be used in MiM
   * @date Jul 07, 2025  
  */
/**
